...
avaliacaoDesvio = group_by(series8temporadas,series_name) %>% 
    summarise(Desvio=round(sd(user_rating), 2)) 
knitr::kable(avaliacaoDesvio[order(avaliacaoDesvio$Desvio),], col.names = c("Série", "Desvio Padrão"))
```
...
```
  ...
  Outra maneira de vizualizar esses dados é utilizando box plot para notar a dispersão das notas por temporadas, a variação em geral não passa de 1 ponto de diferença, reforçando a hipótese de que séries grandes mantém a qualidade.
  
#1.1. Existe alguma relação entre o número de votos e o média de avalição nas séries? Existe relação entre o número de episódios e a popularidade da série.

Para responder as duas perguntas, calcularemos a correlação linear entre as variáveis.

* Número de votos e Média de avaliação:
...
* Número de votos e Popularidade da série
```{r}
...
O valores de correlação pra os dois casos foram pequenos, mostrando uma correlação linear positiva fraca.


#2. Como se comporta o número de votos por episódio ao decorrer das temporadas?

  Aqui estamos interessados em avaliar se a populariade da série aumenta com o decorrer das temporadas.
...
supernatural %>%  ggplot(aes(x = season_ep, y = user_votes, color = season)) + 
  geom_line() + 
  geom_point() +
  facet_wrap(~ as.integer(season)) + 
  xlab("Episódio") +
  ylab("Numero de votos")
```

>> O gráfico abaixo é uma opção de visualização que facilita a comparação entre temporadas e permite observar de maneira geral o número de votos.

```{r}
supernatural %>%  ggplot(aes(x = series_ep, y = user_votes, color = season)) + 
  geom_line() + 
  geom_point() +
  xlab("Episódio") +
  ylab("Numero de votos") 
...
>> Removi o atributo cor do gráfico abaixo pois me pareceu que ele não adicionava nenhuma informação nova ao gráfico nem ajudava a observar as existentes

```{r}
votosPorTemporada = group_by(supernatural,season_ep) %>% summarise(Media = mean(user_votes)) %>% arrange(desc(Media))
votosPorTemporada %>%  ggplot(aes(x = season_ep, y = Media)) + 
...
```
  ...
  Outra análise que podemos fazer é por episódio em cada temporada, se o primeiro episódio é mais popular que o segundo, e segundo que o terceiro e assim por diante em geral. Podemos ver que o primeiro episódio em geral é mais popular doque o resto, mas em geral os episódio mantem uma proximidade no numero de votos. Podemos notar que o número do episódio não influencia muito em sua popularidade, exceto pelo primeiro que o último.

```{r}
votosPorTemporada = group_by(episodes,season_ep) %>% summarise(Media = mean(user_votes)) %>% arrange(desc(Media))
votosPorTemporada %>%  ggplot(aes(x = season_ep, y = Media)) + 
...
```
  ...
  Analisando o gráfico acima podemos ver que quanto maior o número de episódio por temporada mais impopular ela fica. Séries com temporadas muito grandes tendem a perder o público. Podemos concluir que séries com muitos epísódios são menos populares.

#2.2. As melhores séries são maiores que a piores?
...
#2.3. O tamanho da temporada influencia na avaliação?

  Para responder essa questão foram pegas as 15 maiores e menores temporadas.
```{r}
...
```
  ...
  Pelos gráficos podemos ver que temporadas bem pequenas tendem a variar muito sua avalição, enquanto temporadas muito grandes tem uma variação menor ficando com a nota em médio perto de 7.5.
